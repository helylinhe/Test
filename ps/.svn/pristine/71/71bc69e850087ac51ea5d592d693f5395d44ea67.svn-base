...
$(function() {
	...
	$('#search').click(function() {
		...
		var param = {
			"map.flupStatus" : $('#search_flup_status').combobox('getValue'),
			"map.deptCode" : dept_code,
			"map.name" : $('#search_name').val(),
			'map.outHospitalDateStart': $("#outHospitalDateStart").val(),
			'map.outHospitalDateEnd': $("#outHospitalDateEnd").val(),
			"map.inpatiNumber" : $('#search_inpati_number').val(),
			"map.patiSerial":$('#search_pati_serial').val(),
			'map.unitCode' : $('#unitCode').val()//所属院区
		}
...
function createDeptTable(){
	...
	/*loadData(path+'/departmentaction!execute.action',
			'dept_tab', null);
	*/
	$.post(path+'/departmentaction!queryUserDept.action',function (data){
		var blm_datapatient_data={"total":data[0].totalRecord,"rows":data[0].rows};
		$("#dept_tab").datagrid('loadData',blm_datapatient_data);
	},'json');
}
function add() {
...
function findBL(row){
	...
	var unitCodeValue = row.unitCode;
	if(unitCodeValue!=null && unitCodeValue=="1"){
		window.open('http://172.31.10.122:8080/EMRView/main.html?mpi=hm_'+row.patiSerial);
	}else{
		$("#bl_html").panel("open"); 
		setInfo('b',row);
		createBlTable();
...
function findDY(row){
	...
	$('#dy_html').panel("open");
	setInfo('d',row);
	$("#Patientname").val(row.patiSerial);
	$("#topatiSerial").val(row.inpatiNumber);
	$("#toPatientname").val(row.name);
	$("#toage").val(row.age);
	$("#todepartment").val(row.department);
	$("#togender").val(row.gender==1?'男':'女');
	$("#tofamilyNumber").val(row.familyNumber);
	$("#torelationMobileNumber").val(row.relationMobileNumber);
	/***
	 * 创建随访表面
	 */
	printFlupName();
...
//打印呀
function printInfo(flupId,flupNames)
{
						
						
						$("#flupNameId").val(flupId);
...
function setInfo(type,row){//rowData.patiSerial,rowData.inpatiNumber,rowData.inpatiSerialNumber
	$("#"+type+"inpatiNumber").html(row.inpatiNumber);
