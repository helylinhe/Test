...
$(function() {
	...
	var param = {
		...
		"map.inpatiSerialNumber":inpatiSerialNumberValue
	};
	$.post(
					...
					function(data) {
						if (data != null && data != '') {
							...
							//打印用
							$("#toPatientname").val(data[0].name);
							$("#togender").val(data[0].gender);
							$("#toage").val(data[0].age);
							$("#topatiSerial").val(data[0].patiSerial);
							$("#tofamilyNumber").val(data[0].familyNumber);
							$("#torelationMobileNumber").val(data[0].relationMobileNumber);
							$("#todepartment").val(data[0].inpatiDiag.department);
							
							//$("#to").val(data[0].);
							//alert(data[0].gender+"------"+data[0].gender+"------"+data[0].age+"------"+data[0].patiSerial+"------"+data[0].mobileNumber+"------"+data[0].name);
							...
							$.post(
											...
											function(data) {
												...
												$("#toflupNames").val(data[0].flupName);
												/*******************************
												 * 异步获取随访模板
												 */
											var param = {
...
function addTr() {
	...
	var flupTimes = (parseInt($("#timenext" + (index - 1)).html()) + 1);
	var html = "<tr>";
	html += "<td>计划随访日期:";
	...
	html += "<td>第<lable id='timenext" + index + "'>"
			+ (parseInt($("#timenext" + (index - 1)).html()) + 1)
			+ "</lable>次随访<lable><button onclick='javascript:open_follow("+flupTimes+");'>随访项</button></lable></td>";
	html += "</tr>";
...
function showNextPlan() {
	...
	if (nextValue != "" && nextValue != null && nextValue == "无") {
		...
		var  time=parseInt($("#times").html()) + 1;
		$("#timenext0").parent().find("lable").last().html("");
		$("#timenext0").parent().find("lable").last().html("<button onclick='javascript:open_follow("+time+");'>随访项</button>");
...
function findDY(){
	...
	$("#flupNameId").val($("#hiddenFlupNameId").val());
	$("#flupnames").val($("#toflupNames").val());
	$("#Patientname").val($("#toPatientname").val());
	$("#Patgender").val($("#togender").val());
	$("#Patage").val($("#toage").val());
	$("#PatpatiSerial").val($("#topatiSerial").val());
	$("#PatfamilyNumber").val($("#tofamilyNumber").val());
	$("#PatrelationMobileNumber").val($("#torelationMobileNumber").val());
	$("#inpatiSerialNumber").val($("#inpatiSerialNumber").val());
	$("#Patdepartment").val($("#todepartment").val());
	$("#showButton").val("");
...
function open_follow(count){
	...
	/*$('#followStudy_dateGrid').datagrid( {
		...
		onLoadSuccess: function () {
            ...
            全选            $("#detailcheckbox").click(function () {
	...
	$('#followStudy_dateGrid').datagrid('unselectAll');
	var param = {
			"map.patiSerial" : $('#patiSerial').text(),
			"map.inpatiNumber" :$('#inpatiNumber').text(),
			"map.inpatiSerialNumber" :$($('#inpatiSerialNumber', parent.document)).val(),
	...
	$.post(path + "/fileIndexAction!findAllFlupDynamic.action",param,function (data){
		$("#followStudy_dateGrid").html(data);
		$("#followStudy_window").panel({title:"第"+count+"次随访项信息设置"});
	},'html');
...
$(function (){
	 ...
	 * 保存
	 */
	$("#saveFlupDynamic").live('click',function (){
		var patiSerial = $("#patiSerial").html();
		var inpatiNumber= $("#inpatiNumber").html();
		var inpatiSerialNumber=$($('#inpatiSerialNumber', parent.document)).val();
		var insuranceNo =$($('#medicalInsuranceCode', parent.document)).val();
		var deptCode =$("#deptCode").val();
		var inHospitalDate=$("#inHospitalDate").html();
		var outHospitalDate=$("#outHospitalDate").html();
		var flupNameId=$("#hiddenFlupNameId").val();
		var flupTimes=$("#flupTimes").val();
		var groupArray = new Array();
		$('input:checkbox[name=cb_flup_group]:checked').each(function (i,n){
			var groupValue = $(this).val();
		    	groupValue+=",";
			    groupArray.push(groupValue);
		});
		var param={"map.patiSerial":patiSerial,"map.inpatiNumber":inpatiNumber,
				"map.inpatiSerialNumber":inpatiSerialNumber,"map.insuranceNo":insuranceNo,
				"map.deptCode":deptCode,"map.inHospitalDate":inHospitalDate,
				"map.outHospitalDate":outHospitalDate,
				"map.flupNameId":flupNameId,
				"map.flupTimes":flupTimes,
				"map.group":groupArray}
		if(groupArray.length>0){
		$.post(path+"/fileIndexAction!addFlupDynamic.action",param,function (data){
			if(data>0){
				alert('保存成功');
	...
	$("#closeFlupDynamic").live('click',function(){
		$("#followStudy_window").window('close');
	});
});
/**
