...
/*	$('#search_out_hospital_date').click(function() {
		WdatePicker( {
			maxDate : '%y-%M-%d'
		});
	});*/
...
function createTable() {
	$('#blm_grid').datagrid( {
		...
		toolbar : [ 
		{
			...
			parent.loadBingDetail(rows[0].patiSerial,rows[0].inpatiNumber,rows[0].inpatiSerialNumber);
			}
		}
		],onDblClickRow:
...
function createHighcharts(data, title, name) {
	...
	if (data.length == 0) {
		...
		for ( var int = 0; int < data.length; int++) {
			categories.push(data[int].inspectDate);
			seriesDate.push(parseFloat(data[int].inspectResult));
		}
	}
