...
$(function() {
	...
	/*
	 * Sidebar.init( { location : 'left',// 初始化sidebar的位置为左侧 Icon : leftMenu });
	 */
	Navbar.init();// 初始化导航条
	Arrows.init();// 初始化分页箭头
			...
			$("a", themsSetting)
					.click(
							function() {
								...
								var src = themeid.substring(themeid
										.indexOf("_") + 1, themeid.length);
								var h = $(window).height();
								var w = $(window).width();
								$("#zoomWallpaper").attr("src",
										"images/bg/" + src + ".jpg").width(w)
										.height(h);
...
Navbar = function(me) {
	...
	bindSwitchDesktopAnimate : function(t, c) {// 切换动画事件 t 目标桌面 c当前桌面
		...
		var cdesk = Deskpanel.desktopsContainer.find(".desktopContainer[index="
				+ (thisPage - 1) + "]");
		...
		var idesk = Deskpanel.desktopsContainer.find(".desktopContainer[index="
				+ (t - 1) + "]");
		...
		openApp : function(id, title, url, icon, jsonSonMenu, width, height) {// 打开应用窗体-------------------------------------------------------------------------
			...
			if (taskItem.length == 1) {
				...
				var sonfile = "<a id=\"sonfile_a"
						...
						+ "<img width=\"32\" height=\"32\" border=\"0\" src=\"icon/min/"
						+ icon + "\" />" + "</div>"
			...
			bindEvent : function() {// 绑定事件
				this.box.click(function(e) {
					...
					var sonMenu = _this.attr("sonMenu");// 获取子菜单
						...
						var jsonSonMenu = eval("(" + sonMenu + ")");// 将json格式的字符串转换为json
						Windows.openApp(id, title, url, icon, jsonSonMenu, 700,
								500);
					});
			}
		});
